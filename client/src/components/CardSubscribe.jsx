...
export default function CardSubscribe({ anime }) {
  ...
  const handlerWatched = (id) => {
    dispatch(watchedSubscribe(id));
  };

  const handlerUpVote = (id) => {
    dispatch(neutralVoteSubscribe(id));
    dispatch(upVoteSubscribe(id));
  };

  const handlerDownVote = (id) => {
    dispatch(neutralVoteSubscribe(id));
    dispatch(downVoteSubscribe(id));
  };

  const handlerNeutralVote = (id) => {
    dispatch(neutralVoteSubscribe(id));
  };
  ...
  return (
    ...
    >
      <div className="score absolute bg-[#E2A171] p-2 right-0 top-0 rounded-tr-2xl rounded-bl-xl">
        <p className="font-medium text-white">{anime.score}</p>
      </div>
      <div className="card-body px-7 h-full w-full bg-gradient-to-t from-[#2D2D2D] rounded-2xl">
        <div className="mt-auto text-white">
          ...
          <h2 className="font-bold text-xl mt-2 line-clamp-2">{anime.title}</h2>
